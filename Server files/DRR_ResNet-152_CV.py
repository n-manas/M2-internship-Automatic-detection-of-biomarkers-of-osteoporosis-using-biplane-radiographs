...
# For data augmentation (not applied in the end)
import random
import math
...
for train_index, val_index in kfold.split(np.zeros(len(labels))):
    ...
    with open(root_path + 'DRR_ResNet-152_trainHistoryDict_fold_' + str(i), 'wb') as file_pi:
    ...
    # Save the model
    filepath = root_path + '/DRR_ResNet-152_model_simple_fold_' + str(i)
    ...
    with open(root_path + 'DRR_ResNet-152_testResultsDict_fold_' + str(i), 'wb') as file_pi:
            pickle.dump(results, file_pi)
    print("Generate predictions for all test samples")
    predictions = model.predict(ds_val)
    print("predictions shape:", predictions.shape)
    with open(root_path + 'DRR_ResNet-152_testPredictions_fold_' + str(i), 'wb') as file_pi:
    ...
    with open(root_path + 'DRR_ResNet-152_finetuned_trainHistoryDict_fold_' + str(i), 'wb') as file_pi:
            pickle.dump(history_finetuned.history, file_pi)

    filepath = root_path + '/DRR_ResNet-152_finetuned_fold_' + str(i)
    save_model(model, filepath)
    ...
    results = model.evaluate(ds_val, batch_size=batch_size)
    print("test loss, test acc:", results)
    with open(root_path + 'DRR_ResNet-152_finetuned_testResultsDict_fold_' + str(i), 'wb') as file_pi:
            pickle.dump(results, file_pi)
    print("Generate predictions for all test samples")
    predictions = model.predict(ds_val)
    print("predictions shape:", predictions.shape)
    with open(root_path + 'DRR_ResNet-152_finetuned_testPredictions_fold_' + str(i), 'wb') as file_pi:
            pickle.dump(predictions, file_pi)
